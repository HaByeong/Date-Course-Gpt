...
public class DateCourseRequest implements Serializable{
    ...
    public String toPromptString(){
        return "\"---\" 아래의 토픽에 대해 예를 들어줘."
               ...
               + "- Format : 리스트"
               + "\"---\""
                +"1. 사용자가 입력한 금액 " + userChat +"을 기반으로 데이트 코스를 추천합니다. 이때, 추천하는 장소 반드시!! 위에 보냈던 장소 리스트에서 랜덤으로 선정되며, 최소 2가지 최대 4가지의 장소를 추천해줘야함. 또한, 사용자의 예산을 초과하지 않도록 조심스럽게 계획됩니다."
                ...
                +"3. 다른 말 없이 코스 추천, 잔여 비용만 딱 출력해주도록 해줘"
                +"4. 선택된 코스의 비용은 사용자가 입력한 총 금액에서 빼고, 남은 금액을 '잔여 비용:'이라는 형식으로 알려드립니다. 이 금액은 사용자가 추가로 데이트 코스를 요청할 때 참조됩니다."
                +"5. 만약 사용자가 남은 금액으로 추가적인 장소 추천을 원하면, 남은 금액을 초과하지 않는 범위에서 리스트 내 임의의 장소를 추천 후 잔여 비용을 출력합니다."
                +"위 조건들을 반드시 지켜서 출력해줘";
    }
}
