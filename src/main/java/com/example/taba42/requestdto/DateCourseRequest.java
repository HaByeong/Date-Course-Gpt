...
public class DateCourseRequest implements Serializable{
    ...
    public String toPromptString(){
        return "\"---\" 아래의 토픽에 대해 예를 들어줘."
               ...
               + "- Format : 리스트"
               + "\"---\""
               + "넌 데이트 코스를 추천해주는 컨설턴트야. 사용자가 입력하는 금액인 " +userChat+ "을 토대로 데이트 코스를 생성해주되, 반드시!!!! '리스트에 있는 장소들로 가격에 맞게 '매번 랜덤으로 다르게 추천'해 줘야 해' 그리고 반드시!! '사용자가 입력한 금액 안으로 맞춰서 추천해야 해(잔여 비용이 -가 되면 안 돼)'"
                ...
                + "또한, 반드시!! '코스를 선택하면 선택한 코스의 비용을 기존 사용자가 입력한 금액에 빼서 남은 금액을 입력해줘. 이때 형식은 \"잔여 비용: \" 이렇게 다른 문장없이 잔여 비용만 출력해줘 => 단, 반드시 잔여 비용 출력은 사용자가 다시 코스를 추천해달라했을시 그때 비용으로 다시 게산해야해'"
                + "그리고 이후에 사용자가 잔여 비용으로 추가적인 장소 추천을 원하면 잔여 비용을 넘지 않는 장소선에서 임의로 추천해줘";
    }
}
